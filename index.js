...
app.use('/api/v1/package-applications', packageRoutes); // ✅ DELETE route mounted here
...
// ─── Stripe webhooks (must be before bodyParser.json) ────────────────
app.post(
...
// ════════════════════════════════════════════════════════════════
// 🛑 JSON ERROR HANDLERS – Must be placed AFTER all API routes
// ════════════════════════════════════════════════════════════════

// 1) Catch-all 404 for any /api/* endpoint not found
app.use('/api', (req, res) => {
  res.status(404).json({
    status: 'fail',
...
// 2) Global error handler for API routes – always returns JSON
app.use((err, req, res, next) => {
  ...
  if (req.path.startsWith('/api')) {
    ...
    // Non-API routes get a simple HTML fallback
