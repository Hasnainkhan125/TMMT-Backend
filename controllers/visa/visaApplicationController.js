...
// ─── ✅ FIXED: updateApplicationStatus with proper chat initialization ───
exports.updateApplicationStatus = catchAsync(async (req, res, next) => {
  ...
  if (!application.metadata.chatHistory) {
    application.metadata.chatHistory = [];
  }
  ...
  // Add status update to chat history
  application.metadata.chatHistory.push({
    type: 'system',
    content: `Application status changed from ${oldStatus} to ${newStatus}`,
    userId: req.user._id,
    createdAt: new Date(),
    by: 'system',
    role: 'system',
  ...
  try {
    await AuditLog.createEntry({
      ...
      diff: { before: { status: oldStatus }, after: { status: newStatus }, changes: [{ field: 'status', old_value: oldStatus, new_value: newStatus }] },
      request_id: req.headers['x-request-id'] || (Date.now().toString()),
      result: 'success'
    });
  } catch (e) { }
  ...
  try {
    ...
    const sponsorId = application.sponsor.userId?.toString?.() || application.sponsor.userId;
    wsServer?.sendToUser(sponsorId, 'application_status_updated', {
      type: 'success',
...
// ─── ✅ FIXED: addComment with proper initialization and response ───────────
exports.addComment = catchAsync(async (req, res, next) => {
  console.log('📝 addComment called with body:', req.body);
  ...
  // Get the comment from request body - support multiple field names
  const commentContent = req.body.comment || req.body.text || req.body.message;
  ...
  const isAdmin = req.user.role === 'amer' || req.user.role === 'admin';

  // Add comment to chat history
  const newComment = {
    _id: new Types.ObjectId(),
    type: isAdmin ? 'amer' : 'user',
  ...
  application.metadata.chatHistory.push(newComment);
  ...
  // Send WebSocket notification to the sponsor
  try {
    const app = require('../../index');
    const wsServer = app.get('wsServer');
    const sponsorId = application.sponsor.userId?.toString?.() || application.sponsor.userId;
    
    // Format the comment for WebSocket response
    const commentToSend = {
      _id: newComment._id,
      text: commentContent,
    ...
    console.log('📡 Sending WebSocket comment:', commentToSend);

    wsServer?.sendToUser(sponsorId, 'comment_added', {
      type: 'comment',
      message: `New message from ${isAdmin ? 'Admin' : 'You'}`,
      applicationId: application._id.toString(),
      timestamp: new Date(),
    ...
    await Notification.create({
      ...
      type: 'comment',
      title: 'New Message',
      message: `New message about your application: ${commentContent.slice(0, 100)}${commentContent.length > 100 ? '...' : ''}`,
      metadata: {
...
// ─── ✅ FIXED: requestDocuments with proper chat initialization ──────────
